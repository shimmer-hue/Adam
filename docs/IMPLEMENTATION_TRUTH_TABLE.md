...
| Session-start legacy knowledge graph normalization | Implemented | session start now runs a bounded graph-normalization pass over unresolved legacy knowledge rows, persists missing information entities/typed edges into the graph, records `session_graph_normalization` in session metadata, and emits `GRAPH_NORMALIZATION` trace events; the Adam-identity MLX review hook is code/test-covered, while live-model quality remains bounded by the current prompt/schema |
