...
| React observatory shell | Implemented | checked-in Vite bundle served by Python shells and copied into static exports; the browser shell now opens into `Overview`, `Data Laboratory`, and `Preview` top tabs instead of the older strip-plus-three-column shell |
| Gephi-style observatory workspace grammar | Implemented | `Overview` is graph-first with deterministic left/center/right docks, `Data Laboratory` is a dedicated spreadsheet workspace, `Preview` is a separate final-render/export stage, and `Reset layout` restores the default workbench arrangement |
...
| Browser Data Lab and Gephi export interoperability | Implemented | `Data Laboratory` now provides dedicated `Nodes` / `Edges` tables with sortable columns, show/hide column controls, safe bulk selection/export actions, graph↔table selection sync, explicit export-scope selection, and Gephi-accepted graph downloads (`gexf`, `graphml`, `gdf`, `gml`, `graphviz dot`, `pajek net`, `netdraw vna`, `ucinet dl`, `tulip tlp`, `tgf`); `Assemblies`-derived ontology exports carry typed knowledge relations plus explicit memode membership edges rather than only the meme-support slice |
...
| Preview before commit | Implemented | live API + server tests cover preview semantics, the React observatory exposes preview/commit controls plus compare patch rendering, and the separate `Preview` workspace keeps export styling distinct from mutation preview |
