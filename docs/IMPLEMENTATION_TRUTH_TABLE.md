...
| Layered observatory graph payload | Implemented | authoritative `semantic_*`, `assembly_*`, `runtime_*`, `assemblies`, `cluster_summaries`, and `active_set_slices` planes |
| Auto-derived typed knowledge relations | Implemented | ingest/runtime indexing now persists bounded rule-derived `AUTHOR_OF`, `INFLUENCES`, and `REFERENCES` edges with provenance instead of flattening every knowledge relation to `CO_OCCURS_WITH` |
