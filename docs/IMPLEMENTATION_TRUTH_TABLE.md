...
| PDF ingest | Implemented | validated on `eden_whitepaper_v14.pdf`; identical completed ingests are now reused by SHA, large PDFs stream page extraction into chunk storage, and failed ingests mark the document row `failed` instead of leaving a silent stale `processing` record |
