...
| Session-start graph wake-up audit | Implemented | session start runs a bounded two-phase audit: legacy knowledge normalization persists missing information entities/typed edges, and behavior taxonomy audits turn-attached Adam/feedback bundles to strengthen first-order behavior memes and materialize bounded behavior memodes; session metadata records `session_graph_normalization`, `session_graph_taxonomy`, and `session_graph_wakeup`, while traces emit `GRAPH_NORMALIZATION`, `GRAPH_TAXONOMY_AUDIT`, and `GRAPH_WAKEUP_AUDIT`; the Adam-identity MLX review hooks remain code/test-covered but are now explicit opt-in via `EDEN_ENABLE_MLX_WAKEUP_REVIEW=1`, leaving the default live session-start path deterministic for stability |
