...
| Browser Data Lab and Gephi export interoperability | Implemented | node/edge tables, bulk selection handoff, CSV/JSON selection export, and Gephi-accepted graph downloads (`gexf`, `graphml`, `gdf`, `gml`, `graphviz dot`, `pajek net`, `netdraw vna`, `ucinet dl`, `tulip tlp`, `tgf`) are browser-visible and non-authoritative; `Assemblies` mode now exports typed knowledge relations plus explicit memode membership edges rather than only the meme-support slice |
