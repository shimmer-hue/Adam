...
def cmd_export(args) -> int:
    runtime = build_runtime(args)
    experiment_id = args.experiment_id or runtime.primary_experiment()["id"]
    result = runtime.export_observability(experiment_id=experiment_id, session_id=args.session_id)
    print(json.dumps(result, indent=2))
    return 0
...
def cmd_observatory(args) -> int:
    ...
    if args.open:
        experiment = runtime.primary_experiment()
        target_url = status["url"]
        if experiment is not None:
            runtime.export_observability(experiment_id=experiment["id"], session_id=None)
            target_url = f"{status['url']}{experiment['id']}/observatory_index.html"
